...
int main(int argc, char **argv) {
  ...
  cudaProfilerStart();
  cudaMallocManaged((void**)&A, vec_sizeA);
  cudaMallocManaged((void**)&B, vec_sizeB);
  cudaMallocManaged((void**)&C, vec_sizeC);
  ...
  gemm<<<dim3(Dgx, Dgy, 1), dim3(Dbx, Dby, 1)>>>(A, B, C, m, k, k, n);
  ...
  int flag = 0;
  for (int i=0; i < m*n; i++) {
    if (abs(C[i]-resultRef[i]) > max_diff) {
      printf("Error!\n");
